...
from Enviroment.for_special_training.training import Training_Manager
...
class SimulationManager():
    ...
    def learning(self):
        q_tables, rewards = Training_Manager().train_agents()
        if cache_results:
            self.cache_tables(q_tables)
        if show_graph:
            self.build_plot(rewards)

    def cache_tables(self, q_tables: Dict, cache_dir: str = "cache", try_dir_base: str = "progon_") -> None:
        ...
        for agent_id, agent_q_table in q_tables.items():
            table_agent_path = os.path.join(new_folder, f"table_{agent_id}.json")
            table = self.serialize_keys(agent_q_table)
...
class SimulationManager:
    ...
    def run_simulation(
        ...
        # определяет какая ветка будет запущена:
        #  - обучение патрона или альтруиста,
        #  - демонстрация их поведения,
        #  - демонстрация среды со случайными действиями итд)
        if learning_flag:
            self.learning()
        else:
            self.load_tables(progon_number)
